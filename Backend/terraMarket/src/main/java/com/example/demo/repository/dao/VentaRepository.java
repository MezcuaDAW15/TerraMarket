...
public interface VentaRepository extends JpaRepository<Venta, Long> {
    ...
    @Query(value = "SELECT v FROM Venta v JOIN v.producto p JOIN v.tienda t WHERE v.activo=true AND t.id = :idTienda AND p.categoriaP.id= :idCategoria AND v.precioVenta=(SELECT MIN(v2.precioVenta) FROM Venta v2 JOIN v2.tienda t2 WHERE v2.producto.id = v.producto.id AND v2.activo = true AND t2.id = t.id)")

    List<Venta> findCheeperByCategoriasTienda(@Param("idCategoria") Long idCategoria,
